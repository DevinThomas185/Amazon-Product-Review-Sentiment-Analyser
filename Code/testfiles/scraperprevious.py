...
def getReviews(asin):
    ...
    while True:  # Iterate until the specific message denoting no more reviews is found
        soup = getHTML(
            "https://www.amazon.co.uk/product-reviews/" + asin + "/ref=cm_cr_arp_d_paging_btm_next_2?pageNumber=" + str(
                i), proxies)
...
def analyseReviewsRuleBased(reviews):
    ...
    analysis['outcome'] = determinationFunction(analysis["pos_percent"] / 100, analysis["neu_percent"] / 100,
                                                analysis["neg_percent"] / 100)
    return analysis
...
def analyseReviewsAutomatic(reviews):
    ...
    X_train, X_test, y_train, y_test = train_test_split(text_counts, train['reviews.rating'].values.astype('U'),
...
def searchHTML(asin, soup, analysis_type):
    ...
    # t = datetime.now()
    # product_json["asof"] = str(t.time().replace(microsecond=0)) + " on " + str(t.date())
...
def getInformation(request, asin=None):
    ...
    product_json = searchHTML(asin.upper(), soup, analysis_type)
